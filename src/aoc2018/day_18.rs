...
impl Solution for AoC2018_18 {
    fn part_one(&self) -> String {
        ...
        (0..10).for_each(|_| state = update(&state));
    ...
    for (row, arr) in input.iter().enumerate() {
        ...
        for (col, value) in arr.iter().enumerate() {
            ...
            adj.iter().for_each(|x| match x {
                Acre::Trees => trees += 1,
                Acre::OpenGround => open += 1,
                Acre::Lumberyard => lumberyard += 1,
            });
            output[row][col] = next_acre(*value, open, trees, lumberyard);
        }
    }
    output
}

fn next_acre(current: Acre, _open: usize, trees: usize, lumberyard: usize) -> Acre {
    match current {
